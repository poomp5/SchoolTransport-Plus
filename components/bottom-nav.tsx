        ...
        <div className="fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 md:hidden">
            <div className="grid h-full max-w-lg grid-cols-6 mx-auto">
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.href}
                            href={item.href}
                            className={`whitespace-nowrap inline-flex flex-col items-center justify-center px-1 hover:bg-gray-50 ${isActive ? 'text-red-800' : 'text-gray-500'
                                }`}
                        >
                ...
                <button
                    ...
                    className="inline-flex flex-col items-center justify-center px-1 text-gray-500 hover:bg-gray-50"
                >
                    ...
                    <span className="text-[10px]">{locale === 'th' ? 'EN' : 'TH'}</span>
                </button>
            </div>
        </div>
    )
